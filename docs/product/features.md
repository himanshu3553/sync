...
- **Knowledge Base.** Workflows as the assistant sees them — title, description, steps with screenshots and a highlight lightbox — with the approve toggle, a filter-by-recording on both lists (pre-applied when arriving from a recording's page), the AI Agent card (run switch, eligibility, run summary), the Help portal card (publish with a founder-assigned category — independent of copilot approval, per the one-KB-many-audiences model), duplicate warnings folded into one box, the Product knowledge section with its pending updates and its own Approve All, the "Replaced" view, and Video/SOP and Analytics tabs on each workflow — the SOP half exports the workflow as documents, compiled on request from the live steps: a step-by-step guide for people (markdown + annotated screenshots, zipped) and a text-only manual AI agents can operate the product from (routes, expected outcomes, inputs always asked from the user — with a preview of exactly what agents receive).
- **Copilot.** Everything in [§4](#4-going-live), with the live preview.
- **Analytics.** [§6](#6-analytics-and-the-feedback-loop).
- **Help Portal.** The portal manager: the master switch (off by default, enabled behind a recorded acknowledgment that published screenshots become public) with the public and `llms.txt` URLs, every published guide grouped by category with per-guide actions (view, change category, unpublish) and category rename, a bulk "Add guides" flow publishing straight from the Knowledge Base — and the status column where the portal's fail-closed behaviour becomes visible: a workflow detached by a reprocess shows as "Hidden — needs re-review" instead of silently vanishing.
...
- **Portal depth.** The public help portal is live ([§5](#5-studio)): each workspace can publish workflows as categorized, search-indexable help articles at its own `/help/<slug>` site. Every article also serves a machine-readable `.md` sibling, indexed by a per-portal `llms.txt`, so third-party AI agents can learn the product from the same portal. What remains is the track's depth — portal-only presentation editing, type-a-topic articles, retrieval-backed search, theming and custom domains, screenshot PII redaction. → [`landing-page.md`](landing-page.md)
